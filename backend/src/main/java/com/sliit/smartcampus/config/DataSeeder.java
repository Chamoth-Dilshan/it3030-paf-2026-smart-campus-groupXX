    ...
    CommandLineRunner initDatabase(UserRepository userRepository, ResourceRepository resourceRepository) {
        return args -> {
            try {
                ...
                log.warn("Skipping user seed initialization because MongoDB is unavailable: {}", ex.getMessage());
            }
            ...
            try {
                if (resourceRepository.count() == 0) {
                    ...
                    log.info("Seed data loaded: Institutional Resources initialized.");
                }
            } catch (Exception ex) {
                log.warn("Skipping resource seed initialization because MongoDB is unavailable: {}", ex.getMessage());
            }
        };
    }
}
