          ...
          <motion.div style={{ opacity: headingOpacity, scale: headingScale }}>
             ...
             >
                ...
                <div className="flex items-center gap-10">
                   <Link to="/login" className="btn-primary px-8 py-3 font-semibold uppercase tracking-widest text-sm">
                      Launch Portal
                   </Link>
                   <Link to="/availability" className="flex items-center gap-3 text-white font-bold text-xs uppercase tracking-[0.2em] group hover:text-blue-200 transition-colors">
                      Facility Inventory <ArrowRight size={18} className="group-hover:translate-x-2 transition-all text-blue-300" />
                   </Link>
                </div>
             </motion.div>
          </motion.div>
          ...
          <div className="hidden lg:flex flex-col gap-6 items-end justify-center">
             {personas.map((persona, i) => (
                <motion.div 
                   ...
                   whileHover={{ x: -12 }}
                   className="w-[450px] surface-glass-dense rounded-2xl border-slate-200 group cursor-pointer hover:border-slate-300 transition-all p-8"
                >
                   <div className="flex items-start gap-6">
                      <div className="w-14 h-14 bg-gradient-primary rounded-xl flex items-center justify-center text-white group-hover:shadow-lg transition-all shadow-md">
                         {persona.icon}
                      </div>
                      <div>
                         <span className="text-xs font-bold uppercase tracking-wider text-blue-600 mb-2 block">{persona.role}</span>
                         ...
                         <div className="flex items-center gap-2 text-xs font-bold text-slate-600 uppercase tracking-wider group-hover:text-blue-600 transition-colors">
                            {persona.cta} <ArrowRight size={12} className="text-blue-600" />
                         </div>
                      </div>
                   </div>
                </motion.div>
               ...
               <div className="grid gap-8">
                  {intelligenceData.map((intel, i) => (
                     <motion.div 
                       ...
                       className="surface-glass rounded-2xl p-8 flex items-center justify-between group hover:surface-glass-dense transition-all"
                     >
                        <div className="max-w-[60%]">
                           <h4 className="text-xs font-bold text-slate-900 uppercase tracking-widest mb-2">{intel.label}</h4>
                           <p className="text-sm text-slate-600 font-medium leading-relaxed">{intel.desc}</p>
                        </div>
                        <div className="text-right">
                           <div className="text-3xl font-bold text-slate-900 mb-1">{intel.value}</div>
                           <span className="text-xs font-bold text-emerald-600 uppercase tracking-widest">{intel.trend}</span>
                        </div>
                     </motion.div>
                  ))}
               </div>
