        ...
        <div className="bg-slate-50 min-h-screen pt-40 pb-32 px-6 relative overflow-hidden">
            ...
            <div className="max-w-7xl mx-auto relative z-10">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-10 mb-20">
                    <div>
                        <Link to="/dashboard" className="inline-flex items-center gap-2 text-blue-600 font-black tracking-widest uppercase text-[10px] mb-8 hover:gap-3 transition-all">
                            <ArrowLeft size={14} /> System Nexus
                        </Link>
                        <motion.h1 
                            initial={{ opacity: 0, x: -30 }}
                            animate={{ opacity: 1, x: 0 }}
                            className="text-8xl font-bold text-slate-900 leading-tight"
                        >
                            My Registry.
                        </motion.h1>
                    ...
                    <div className="flex items-center gap-4">
                        <div className="surface-glass px-6 py-4 rounded-3xl border border-white/20 shadow-xl flex items-center gap-8">
                           <div className="flex flex-col">
                               <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Active Pool</span>
                               <span className="text-2xl font-bold text-slate-900">{bookings.length} Records</span>
                           </div>
                           <Link to="/bookings" className="btn-primary px-8 py-3 text-xs">
                               + New Entry
                           </Link>
                        </div>
                        <button 
                ...
                <div className="mb-10 flex flex-col md:flex-row gap-6 items-center">
                    <div className="relative flex-1 group">
                        ...
                        <input 
                            ...
                            className="w-full pl-16 pr-8 py-5 bg-white/50 backdrop-blur rounded-[2rem] border border-white shadow-xl text-lg font-bold text-slate-900 focus:ring-8 focus:ring-blue-500/5 focus:border-blue-500 outline-none transition-all"
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                        />
                    </div>
                    <div className="flex gap-4">
                        <button className="p-5 bg-white/50 border border-white rounded-[1.5rem] text-slate-400 hover:text-blue-600 transition-all shadow-xl backdrop-blur">
                            <Filter size={20} />
                        </button>
                    </div>
                </div>
