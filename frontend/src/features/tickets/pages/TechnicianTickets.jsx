...
import { fetchIncidents } from '../services/incidentsSlice';
...
const TechnicianTickets = () => {
  ...
  const { items: incidentItems, status } = useSelector((state) => state.incidents);
  ...
  useEffect(() => {
    const myTickets = incidentItems.filter(isAssignedToCurrentTech);
    const finalTickets = [...myTickets].sort(
      (a, b) => new Date(b.createdAt || b.dateReported) - new Date(a.createdAt || a.dateReported)
    );

    setIncidents(finalTickets);
    setLoading(status === 'loading' && incidentItems.length === 0);
  }, [incidentItems, status, techName, user.id]);
